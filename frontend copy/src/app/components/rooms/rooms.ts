...
export class RoomsComponent implements OnInit, OnDestroy {
  ...
  async openCreateRoomDialog(): Promise<void> {
    const { CreateRoomDialogComponent } = await import('./create-room-dialog');
    const dialogRef = this.dialog.open(CreateRoomDialogComponent, { width: '400px' });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        this.roomsService.createRoom(result).subscribe({
          next: (room) => {
            this.snackBar.open('Sala creada exitosamente', 'Cerrar', { duration: 2000 });
            this.loadRooms();
          },
          error: (error) => {
            this.snackBar.open(error.error?.message || 'Error al crear la sala', 'Cerrar', { duration: 3000 });
          }
        });
      }
    });
  ...
  async openDeleteRoomDialog(room: Room): Promise<void> {
    const { ConfirmDeleteDialogComponent } = await import('./confirm-delete-dialog');
    const dialogRef = this.dialog.open(ConfirmDeleteDialogComponent, { width: '350px' });
    dialogRef.afterClosed().subscribe((confirmed) => {
      if (confirmed) {
        this.roomsService.deleteRoom(room._id).subscribe({
          next: () => {
            this.snackBar.open('Sala eliminada exitosamente', 'Cerrar', { duration: 2000 });
            this.loadRooms();
          },
          error: (error) => {
            this.snackBar.open(error.error?.message || 'Error al eliminar la sala', 'Cerrar', { duration: 3000 });
          }
        });
      }
    });
